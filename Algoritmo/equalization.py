""""
====================================================================
    Algoritmos de Equalização para Sistemas Ópticos Coerentes 
====================================================================

Este módulo implementa algoritmos de equalização adaptativa para
receptores ópticos coerentes. Ele oferece suporte à execução tanto
...
.. autosummary::
   ...
   ddlmsUp               -- Equalizador por Mínimos Quadrados com Decisão Direta (DD-LMS).
   mimoAdaptEq           -- Controlador geral do equalizador adaptativo MIMO 2x2.
   resultados            -- Cálculo e alinhamento das métricas BER/SER/SNR da simulação.
"""
...
def ddlmsUp(x, constSymb, nModes, paramEq):
    ...
    ----------
      ...
      paramEq : parameters - Estrutura contendo parâmetros do equalizador, incluindo:
        ...
        - blockSize : número de amostras por iteração.
...
def mimoAdaptEq(x, paramEq):
    ...
    """
    ------------------------------------------------------------------
    Seleção automática da biblioteca de processamento (CPU ou GPU)
    ------------------------------------------------------------------
    Verifica se o sinal de entrada 'x' é um array CuPy (GPU) ou NumPy (CPU)
    e define dinamicamente qual biblioteca será usada ao longo das operações.
    Caso 'x' esteja na GPU, a constelação 'constSymb' também é convertida
    para o formato CuPy, evitando transferências desnecessárias entre CPU e GPU.
    """
    ...
    # Chama o algoritmo de equalização especificado
...
def resultados(y_CPR_LMS, paramTx, symbTx_, Nsymb):
    """
    Calcula as métricas de desempenho do sistema óptico após equalização e recuperação de portadora.

    Esta função compara os símbolos transmitidos e recebidos após todas as etapas digitais
    (CMA, RDE, DD-LMS, CPR), realizando o alinhamento temporal e normalização de potência,
    e computa as métricas de desempenho principais: BER, SER e SNR.

    Parâmetros
    ----------
    y_CPR_LMS : np.ndarray
        Sinal recebido após equalização e recuperação de fase (CPR).
        Formato: [N_amostras x 2], sendo as colunas 0 e 1 as polarizações X e Y.

    paramTx : parameters
        Estrutura de parâmetros do transmissor (ordem de modulação, tipo de constelação, etc.).

    symbTx_ : np.ndarray
        Símbolos transmitidos para comparação com os símbolos equalizados.

    Nsymb : int
        Número de símbolos processados (para fins de exibição no log).

    Retornos
    --------
    tuple
        - tx (np.ndarray): Símbolos transmitidos, alinhados temporalmente.
        - rx (np.ndarray): Símbolos recebidos (equalizados e compensados).
    """
